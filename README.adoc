...
* Swerve drive DSL and extension functions.

=== `xyz.malefic.frc.swerve`

The `swerve` package provides a complete swerve drive implementation with advanced features:

* `SwerveModule`: Individual swerve module with drive and steer motors using TalonFX and CANcoder.
* `SwerveDrive`: Complete swerve drive subsystem with pose estimation, field-oriented driving, and vision integration.
* `TeleopSwerve`: Teleop command with input shaping, deadband, and slew rate limiting.
* `LockSwerve`, `ResetGyro`, `DriveToAngle`, `TurnToAngle`: Utility commands for various swerve operations.
* DSL extensions for easy configuration and command creation.
...
* `Engu`: Enhanced CANcoder wrapper with swerve-optimized utilities, angle conversions, and calibration methods.
...
The `state` package provides a comprehensive state machine framework that integrates with WPILib's command-based programming:
